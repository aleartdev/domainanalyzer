...
def parse_search(search):
    ...
    INFO["SEARCH"] = search
    _domain_name_ = (
        search.split("//")[-1].split("/")[0] if "//" in search else search
    )
...
def get_information():
    ...
    for function in functions:
        threads_list.append(
            Thread(name=function, target=function, args=(INFO["DOMAIN NAME"],))
        )
...
def analyze():
    ...
    if "ok" not in INFO["STATUS"].lower():
        SUGGESTIONS["warning"].append(
            'Status code not "OK": {}'.format(INFO["STATUS"])
        )
    ...
    if "spf" not in INFO["TXT"].lower():
        ...
        if not any(
            ...
            for host_ in [
                INFO["MX DOMAIN NAME"],
                host_domain(INFO["MX ORGANIZATION"]),
                host_domain(INFO["MXHR"]),
            ]
        ):
    ...
    try:
        if (
            INFO["DOMAIN NAME HOST"] not in INFO["MXHR"]
            and INFO["MX DOMAIN NAME"]
        ):
            SUGGESTIONS["notice"].append(
                "External mail hosted at {} ({})!".format(
                    INFO["MX DOMAIN NAME"], INFO["MX ORGANIZATION"]
                )
            )
    except KeyError:
        pass
...
def output_console():
    ...
    for key, value in sorted(INFO.items()):
        if value:
            ...
            print(
                "{}{}{}{}".format(
                    color["bold"], "{:<17}".format(key), color["end"], value
                )
            )
        else:
    ...
    for error_msg in SUGGESTIONS["error"]:
        print(
            "Error! {}{}{}{}".format(
                color["bold"], color["red"], error_msg, color["end"]
            )
        )
    for warning_msg in SUGGESTIONS["warning"]:
        print(
            "Warning! {}{}{}{}".format(
                color["bold"], color["yellow"], warning_msg, color["end"]
            )
        )
    for notice_msg in SUGGESTIONS["notice"]:
        print(
            "Notice! {}{}{}{}".format(
                color["bold"], color["darkcyan"], notice_msg, color["end"]
            )
        )
...
def get_host(domain):
    ...
    try:
        ...
        INFO["DOMAIN NAME HOST"] = re.findall(
            r"([a-zA-Z0-9_-]*\.[a-zA-Z0-9_]*$)", INFO["HOST"]
        )[0]
    except socket.error:
        INFO["HOST"] = ""
...
def get_txt(domain):
    ...
    try:
        if INFO["IDN"]:
            ...
            domain_dig = domain
        INFO["TXT"] = "\n\t\t ".join(
            [txt.to_text() for txt in dns.resolver.query(domain_dig, "TXT")]
        )
    except (
        socket.error,
...
def get_mx(domain):
    ...
    try:
        ...
        INFO["MX"] = "\n\t\t ".join(
            [mx.to_text() for mx in dns.resolver.query(domain, "MX")]
        )
        # get second word that ends with a dot excluding that dot
        INFO["MX HOST"] = re.findall(r".* (.*).", INFO["MX"])[0]
        INFO["MX DOMAIN NAME"] = re.findall(
            r"([a-zA-Z0-9_-]*\.[a-zA-Z0-9_]*$)", INFO["MX HOST"]
        )[0]
    except (
        socket.error,
        dns.resolver.NoAnswer,
    ...
    if INFO["MXIP"]:
        ...
        try:
            ...
            INFO["MX ORGANIZATION"] = re.findall(
                r"([a-zA-Z0-9_-]*\.[a-zA-Z0-9_]*$)", _whois["emails"][0]
            )[0]
...
def get_whois(domain):
    ...
    try:
        ...
        SUGGESTIONS["error"].append(
            """Python whois UnicodeDecodeError. (Implement fix in readme.md
            https://github.com/freiholtz/domainanalyzer/blob/master/README.md
            )"""
        )
        _whois = []
        if DEBUG:
    ...
    try:
        ...
        INFO["TIME MOD DELTA"] = round(
            (_detla_datetime.seconds / 3600 / 24) + float(_detla_datetime.days),
            2,
        )
    except (KeyError, TypeError):
        INFO["TIME MODIFIED"] = ""
...
def get_statuscodes(domain):
    ...
    try:
        ...
        try:
            INFO["STATUS CODE"] = "{} / {}".format(
                html.getcode(), requests.status_codes._codes[html.getcode()][0]
            )
